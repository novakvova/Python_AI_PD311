...
CSV_FILE = "eur_uah.csv"

if not os.path.exists(CSV_FILE):
    print("⚙️  Файл eur_uah.csv не знайдено — завантажую курс з НБУ...")
    end_date = datetime.now().date()
    start_date = end_date - timedelta(days=365)

    url = f"https://bank.gov.ua/NBU_Exchange/exchange_site?start={start_date.strftime('%Y%m%d')}&end={end_date.strftime('%Y%m%d')}&valcode=eur&sort=exchangedate&json"
    data = requests.get(url).json()

    df = pd.DataFrame(data)[["exchangedate", "rate"]]
    df.columns = ["date", "eur_uah"]
    df["date"] = pd.to_datetime(df["date"], format="%d.%m.%Y")
    df["eur_uah"] = pd.to_numeric(df["eur_uah"], errors="coerce")

    df.to_csv(CSV_FILE, index=False, encoding="utf-8")
    print("✅ Файл eur_uah.csv створено успішно!")

# ---------- 2. Читання CSV ----------
df = pd.read_csv(CSV_FILE)
df["date"] = pd.to_datetime(df["date"], errors="coerce")
df = df.dropna(subset=["date", "eur_uah"])
...
plt.plot(ts["date"], ts["value"], label="Фактичний курс", color="green")
plt.plot(df_forecast["date"], df_forecast["forecast"], marker="o", linestyle="--", color="red", label="Прогноз")
plt.axvline(ts["date"].iloc[-1], color="gray", linestyle=":", label="Кінець історії")
plt.xlabel("Дата")
plt.ylabel("Курс EUR → UAH")
plt.title("Прогноз курсу EUR → UAH на 10 днів вперед")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
